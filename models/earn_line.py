...
class EarnLine(models.Model):
        ...
        for rec in self:
            if rec.time_start < 0 or rec.time_start >= 24:
                raise ValidationError(
                    _("Invalid start time: %s") % rec.time_start)
    ...
    def _check_date_start_end(self):
        for rec in self:
            if rec.date_end < rec.date_start:
                raise ValidationError(
                    _("The end date must always be greater than the start date for: %s") % rec.name)
    ...
    def _check_amount(self):
        for rec in self:
            if rec.amount <= 0:
                raise ValidationError(
                    _("The earn amount must always be greater than 0 for: %s") % rec.name)

    def _valid_field_parameter(self, field, name):
        """Sobreescribir para permitir parámetros personalizados"""
        if name == 'force_model':
            return True
